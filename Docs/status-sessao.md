...
**Pendente de verdade agora**: só falta a task #85/#86 de deploy — as Edge Functions (`wa-webhook`, `otp-solicitar`, `route-cost`, `wa-vincular`) têm todo o código instrumentado (logs de erro + o trial de cálculo, ambos no mesmo `wa-webhook/index.ts`) mas ainda não foram reimplantadas no Supabase. Sem o deploy, nada disso roda em produção ainda.
