...
## Atualização — 08/09 (5): aba "Saúde do sistema" no painel admin

Motivação: Raphael mostrou o painel admin de outra aplicação (PDF, "Painel administrativo — Portal Educacional") com vários indicadores de saúde técnica e perguntou o que faria sentido ter aqui. Análise: os bugs que realmente nos pegaram de surpresa até agora (recursão infinita de RLS, hook de claims sobrescrito) não deixavam nenhum rastro visível no admin antes de alguém reportar — não tínhamos telemetria de erro nenhuma. Proposta aceita ("sim"): 4 cards cobrindo exatamente essa lacuna.

**Restrição de infraestrutura descoberta durante o planejamento**: os logs das Edge Functions (`edge_logs`/`function_edge_logs`) só são acessíveis pela Management API do Supabase — não tem acesso via SQL (Postgres não enxerga) nem do lado do app. Então um card de "erros técnicos" não dá pra alimentar lendo esses logs direto. Solução: instrumentar cada Edge Function pra gravar seus próprios erros na tabela `app_log` já existente (mesma tabela que a aba Auditoria já lê, com RLS por `is_admin_ativo()`).

**Instrumentação nas 4 Edge Functions** (`wa-webhook`, `otp-solicitar`, `route-cost`, `wa-vincular`): cada uma ganhou um helper `logErro(source, message, context)` que grava em `app_log`, sempre dentro do seu próprio try/catch (uma falha ao logar nunca derruba o fluxo principal). Pontos instrumentados são falhas operacionais de verdade, não todo `console.error` (vários eram degradações graciosas, tipo "seguindo mesmo assim"): falha de envio de WhatsApp (mensagem e lista), falha de status "failed" reportado pela Meta, falha ao chamar `route-cost`, falha ao gravar `wa_freight_query`, falha do RPC `registrar_bloqueio_otp` (fallback), falha real de envio do OTP via GoTrue, falha de conexão/erro da Google Routes API, falha ao gravar `wa_vinculo`. Além disso, o handler `Deno.serve` de cada uma das 4 functions agora tem um try/catch de topo (antes só o `wa-webhook` não tinha nenhum — qualquer exceção não prevista virava um 500 sem deixar rastro nenhum).

**Migration** (`20260908150000_saude_do_sistema_funcoes_e_rls.sql`): duas funções novas, `saude_jobs_rollup()` (última execução de cada job de `pg_cron`, com heurística de "atrasado" = não roda há mais de 2h) e `saude_banco()` (tamanho do banco + conexões ativas via `pg_stat_activity`) — ambas `SECURITY DEFINER` porque `authenticated` não tem `USAGE` no schema `cron` nem enxerga `pg_stat_activity` de outras sessões; ambas gated internamente por `is_admin_ativo()` (mesmo padrão do resto do admin — chamada por não-admin só retorna vazio). Também adicionada `otp_bloqueio_select_admin`: essa tabela tinha RLS habilitado mas ZERO policies (bloqueio total, nem admin conseguia ler) — necessário pro card de alertas contar bloqueios ativos.

**Frontend**: `carregarSaudeSistema()` em `data/admin.ts` busca os 4 sinais em paralelo (contagem de erro por fonte nas últimas 24h, jobs, banco, bloqueios ativos). Nova tela `admin/pages/SaudeSistema.tsx` — aba "Saúde do sistema" no menu — com 4 cards: alertas abertos (computado no cliente: jobs atrasados + bloqueios ativos + pico de erro ≥20/24h), erros técnicos por fonte, jobs de rollup com status, banco (tamanho + conexões). Novas classes CSS `admin-tag-bom`/`admin-tag-ruim`/`admin-lista-simples`.
...
**ATUALIZAÇÃO 10/09 — decisão tomada e implementada:** Raphael escolheu a **estratégia B**, com um refinamento: depois de devolver o cálculo (estimativa com perfil padrão), mandar o link de cadastro já na mesma mensagem (não numa mensagem separada depois) — é o momento de maior interesse da pessoa. Ver checkpoint completo abaixo ("CHECKPOINT — 10/09").

## CHECKPOINT — 10/09: trial de cálculo via WhatsApp pra número desconhecido (em validação, sandbox de execução travado)

**O que foi implementado** (`supabase/functions/wa-webhook/index.ts`):

- `tratarPedidoDeCalculo` agora diferencia três casos ao consultar `motoristas` por `telefone_e164`:
  1. **Tem conta, não vinculou o WhatsApp** (`motorista && !motorista.canal_wa_ativo`) — comportamento inalterado, mensagem de sempre pedindo pra vincular pelo app.
  2. **Número não existe em `motoristas` (trial, estratégia B)** — se o pedido já veio completo (origem/destino/valor) e com confiança mínima, calcula na hora com `PERFIL_CUSTO_DEFAULT` (mesmo perfil-padrão já usado internamente pra motorista vinculado sem perfil próprio) e devolve o resultado com um rodapé diferente: "(estimativa com um caminhão padrão — cadastre o seu em instantes pra ter o valor exato do SEU caminhão)" + "🚀 Gostou? Cadastre-se grátis: {URL_APP}/entrar". Se faltou dado, manda uma mensagem de boas-vindas explicando o que é o bot (em vez do "vincule seu WhatsApp", que não fazia sentido pra quem nunca teve conta) + link de cadastro.
  3. **Motorista vinculado normalmente** — fluxo de sempre, sem mudanças.
- `calcularEResponderFrete` agora aceita `motoristaId: string | null` — quando `null` (trial), pula a busca em `caminhao_perfil` (não existe `user_id` pra buscar) e usa `PERFIL_CUSTO_DEFAULT` direto; grava em `wa_freight_query` com o novo status `calculado_anonimo` em vez de `calculado`; dispara `registrarEventoAnalytics` com `event_name: "simulation_run_anonimo"` e `actor_id: null` em vez de `"simulation_run"`.
- `registrarEventoAnalytics` teve o tipo de `actorId` mudado de `string` pra `string | null` (coluna já era nullable no banco) e o `eventName` passou a aceitar `"simulation_run" | "simulation_run_anonimo"`.
- Novo status `nao_cadastrado` em `wa_freight_query` pra quando o número desconhecido manda mensagem incompleta (distinto de `nao_vinculado`, que é "tem conta mas não linkou").
- **Migration aplicada no banco e salva no repo**: `supabase/migrations/20260910120000_wa_freight_query_status_trial_anonimo.sql` — dropa e recria o CHECK constraint de `wa_freight_query.status` incluindo os dois status novos (`calculado_anonimo`, `nao_cadastrado`).
- **Confirmado, não corrompe o gate de validação do MVP**: `v_journey_completion` já filtra `actor_id IS NOT NULL` antes de contar jornada completa — eventos anônimos (`actor_id: null`) não entram na contagem das 160 jornadas.
- **Bug de tipagem corrigido na revisão manual** (não rodei `tsc` ainda, ver "Pendente" abaixo): a linha do perfil usava a variável `anonimo` (booleano calculado separadamente) em vez de checar `motoristaId == null` direto — TypeScript não propaga a narrowing de tipo através de uma variável booleana intermediária, então dava erro de tipo em `buscarPerfilOuDefault(motoristaId)`. Corrigido pra `const perfil = motoristaId == null ? PERFIL_CUSTO_DEFAULT : await buscarPerfilOuDefault(motoristaId);`.

**RESOLVIDO — 10/09 (continuação):** o sandbox de execução (onde eu rodo `tsc`/`vite build`/bash) ficou travado a sessão inteira (~12 tentativas, erro de montagem `RPC error... Plan9 share not mounted`) e não voltou a funcionar — segue travado, é um problema de infraestrutura da ferramenta, não do projeto. Contornado assim: pedi pro Raphael rodar a validação direto no terminal dele.
- Achado no caminho: `apps/web` deu 26 erros de `tsc --noEmit --strict` na primeira tentativa do Raphael, TODOS "Cannot find module '@rode/calc'" — mas isso não tinha nada a ver com a mudança de hoje (nenhum dos arquivos com erro é o `wa-webhook`). Causa: o `node_modules` local dele estava com o link do workspace `@rode/calc` quebrado (`npm install` normal não recria symlink de workspace quando o `node_modules` já existe num estado inconsistente). Corrigido apagando `node_modules` (raiz, `apps/web`, `packages/rode-calc`) e rodando `npm install` de novo do zero — depois disso, `tsc --noEmit --strict` em `apps/web` ficou limpo (0 erros).
- Fiz revisão manual completa do diff do `wa-webhook/index.ts` (já que não consegui rodar o `tsc` do lado Deno) — sem achados além do bug de narrowing já corrigido acima. Risco residual considerado baixo.
- **Commit e push confirmados via `git log --oneline`**: `2b490df` ("Trial de calculo via WhatsApp para numero desconhecido + CTA de cadastro") está no HEAD do `main`, com `origin/main` apontando pro mesmo commit — já está tudo salvo e enviado.

**RESOLVIDO — 10/09 (deploy):** as 4 Edge Functions foram reimplantadas via MCP do Supabase (`deploy_edge_function`), lendo o conteúdo atual de cada uma direto do repo: `wa-webhook` (v38→v39, `verify_jwt=false`, webhook público da Meta), `otp-solicitar` (v25→v26, `verify_jwt=true`), `route-cost` (v25→v26, `verify_jwt=true`), `wa-vincular` (v22→v23, `verify_jwt=true`). Confirmado via `list_edge_functions`: todas `ACTIVE`, `verify_jwt` preservado igual ao que já estava configurado antes (não mudei nenhuma config de auth). Agora está tudo rodando em produção: logs de erro técnico (`app_log`) e o trial de cálculo pra número desconhecido (estratégia B) já valem pra mensagens reais chegando no WhatsApp.

**Tudo que estava pendente desta sessão (08/09 e 10/09) está fechado**: aba Saúde do sistema, instrumentação de erro, trial de cálculo via WhatsApp com CTA de cadastro, e o deploy de todas as 4 functions.
