...
/** Fretes com status "aberto", mais recentes primeiro, com filtros opcionais de UF/tipo de veículo. */
export async function listarFretesAbertos(filtros: FiltrosFrete = {}, limite = 40): Promise<FretePublicado[]> {
  let query = supabase
    .from('fretes_publicados')
    .select(
      'id, empresa_nome, contato_nome, contato_telefone, origem_cidade, origem_uf, destino_cidade, destino_uf, valor_frete_centavos, valor_a_combinar, tipos_veiculo_aceitos, tipos_carroceria_aceitos, status, created_at',
    )
    .eq('status', 'aberto')
    .order('created_at', { ascending: false })
    .limit(limite);

  if (filtros.origemUf) query = query.eq('origem_uf', filtros.origemUf);
