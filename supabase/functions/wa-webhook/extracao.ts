...
- e_pedido_de_busca: true se a mensagem for um pedido do tipo (2), MESMO SEM cidade/rota específica mencionada — a busca não depende de nenhum dado da mensagem, ela usa a cidade base já cadastrada do motorista. Marque true tanto pra pedidos específicos ("tem frete pra SP?", "tem carga saindo daqui pra Curitiba?") quanto pra pedidos genéricos ("tem frete pra mim?", "tem frete disponível?", "tem carga?", "quero ver os fretes", "buscar frete", "o que tem de frete hoje?"). Regra prática: se a mensagem pergunta sobre a EXISTÊNCIA de frete/carga (com ou sem detalhe) e NÃO menciona um valor em reais pra avaliar, é busca. Nunca marque e_pedido_de_frete e e_pedido_de_busca como true ao mesmo tempo.
- Se nenhum dos dois for o caso (saudação, dúvida sobre outro assunto, reclamação), deixe ambos false e os demais campos vazios/zerados. Na dúvida entre "busca genérica" e "nenhum dos dois", se a mensagem mencionar a palavra frete/carga de qualquer forma, prefira classificar como busca — silêncio total é pior que buscar e não achar nada.
